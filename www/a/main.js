...
function init() {
	...
	document.body.addEventListener('htmx:configRequest', function (evt) {
		const te = evt.detail.triggeringEvent;
		const se = evt.detail.triggeringEvent.srcElement;
		let xt = '';
		if (evt.detail.triggeringEvent.type === "click" && se.localName === 'a-w') {
			if (evt.detail.triggeringEvent.altKey) {
				xt = 'a+c'
			} else {
				xt = 'c'
			}
		...
		if (xt) {
			evt.detail.parameters['xt'] = xt;
		}

		evt.detail.parameters['word_id'] = se.attributes['id'].value;
		evt.detail.headers["X-CSRFToken"] = document.querySelector('a-document').attributes['csrftoken'].value;
	});
}
