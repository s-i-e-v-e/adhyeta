...
from mod.root.backend.importers.tp import __get_works_list, __se_raw_base, project_uuids, __ref_to_slug
...
class SeChapter:
    ...
    def from_file(ses: SeSource, sf: str, title: str):
        ...
        x = x.replace('z3998:event', '')
        x = x.replace('z3998:place', '')
        x = x.replace('z3998:frontispiece', '')
        # x = x.replace('<hgroup>', '')
        # x = x.replace('</hgroup>', '')
...
def __build_section_meta(n: sxml.SxmlNode, meta: SectionMeta, foot_notes: dict):
    ...
    # print(meta.label)
    # print(meta.ordinal)
    # print(meta.title)case 'z3998:diary-entry': title, pseudo = '', True
    # print(meta.subtitle)
    ...
    if xs:
        ...
        match n.attrs['type']:
            ...
            case 'z3998:subchapter z3998:postscript': title, pseudo = '', True
            ...
            case _:
                pprint.pprint(n.node(0))
                raise Exception(f'Unknown type `{n.attrs["type"]}`')
...
def __parse_chapter(ses: SeSource, book: SeBook, pc: SeChapter|None, chapter: SeChapter):
    ...
    c_url = __ref_to_slug(slug_title, label, number) + '.xml'
