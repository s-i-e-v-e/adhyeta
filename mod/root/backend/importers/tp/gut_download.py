...
from mod.root.backend.importers.tp import __get_works_list, __gut_raw_base, __ref_to_slug
import re
...
def __split(id: str):
    ...
    for c in doc.all('chapter'):
        ...
        if title:
            ...
            title = sxml.parse('(title @pseudo "" Epigraph)')
        meta.append(title)
        ...
        cdoc.append(c)
        chapter_file = __ref_to_slug(title.value_rec(), title.attrs.get('label', ''), title.attrs.get('n', ''))+'.xml'
