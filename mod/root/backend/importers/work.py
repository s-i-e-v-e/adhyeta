...
def __collect_metadata(n: SxmlNode) -> dict:
    METADATA_NODES = [
        '/document/title',
        '/document/source',
        '/document/copyright',
    ...
    x = {}
    for a in METADATA_NODES:
        label = a.split('/')[-1]
        _, b = sxml.filter_node(n, a)
        if not b:
            if label == 'title':
                x[label] = n.attrs[label]
            continue
        x[label] = sxml.as_str(b.xs[0])
        if label == 'title':
            x[label] = x[label] if x[label] else n.attrs[label]
