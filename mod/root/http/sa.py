...
from mod.root.action.sxml_html import generate_breadcrumb
...
from mod.root.action.sxml_vyakaranam import vyakaranam
...
def get_progress_sxml(uuid: str, user_id: int):
    ...
    return sxml.parse(f"""(meta @hx-swap-oob true @id progress (p (progress @max {unique_words} @value {known_words} @title "{pc}% - {known_words}/{unique_words} words")))""")
...
def get_tab_sxml(uuid: str, selected: int):
    ...
    n = sxml.parse(f"""(div @class tabs @hx-target ".document-container" @hx-swap "outerHTML" (a @href "javascript:void(0);" @hx-post "/sa/" @hx-vals "" {re_selected} Reader)(a @href "javascript:void(0);" @hx-post "/sa/" @hx-vals "" {vy_selected} Grammar))""")

    n.xs[0].attrs["hx-vals"] = re_vals
    n.xs[1].attrs["hx-vals"] = vy_vals
    return n

def render_vyakaranam_page(d: doc.Doc, csrftoken: str, user_id: int, floc: str):
    n = __process(d, csrftoken)
    handle_list_pragma(db, n, floc)

    is_index = floc.endswith("index.sxml")
    if not is_index:
        vyakaranam(n)
        sxml.insert_node(n, "/document/sec", get_progress_sxml(n.attrs["uuid"], user_id), -1)
        sxml.insert_node(n, "/document/sec", get_tab_sxml(d.uuid, 1), -1)

    sxml.insert_node(n, "/document/sec", sxml.parse("(hr)"), -1)
    sxml.insert_node(n, "/document/sec", sxml.parse("(hr)"), 1)
    _, sec = sxml.filter_node(n, "/document/sec")
    assert sec is not None
    tc = sxml.parse("(div @class tab-container)")
    tc.xs.append(sec)
    sxml.replace_node(n, "/document/sec", tc)
...
def render_page(d: doc.Doc, csrftoken: str, user_id: int, floc: str):
    ...
    if not is_index:
        ...
        sxml.insert_node(n, "/document/sec", get_progress_sxml(n.attrs["uuid"], user_id), -1)
        sxml.insert_node(n, "/document/sec", get_tab_sxml(d.uuid, 0), -1)
...
def get_note_form(user_id: int, w: str, xid: str):
    ...
    html = f"""<div hx-swap-oob="true" hash={ww.hash.hex()} id="id_{xid}" class="tag-container">
        <form>
        <p><textarea placeholder="note" cols="40" rows="6" name="note">{note[0] if note else ""}</textarea></p>
