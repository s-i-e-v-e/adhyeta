...
def __get_uuids():
    uuids = [x for x in fs.read_text(f"{fs.get_parent(__file__)}/ramayana.ids").split('\n') if x.strip()]
    uuids.reverse()
    return uuids
...
def get_sarga_list(uuids: list[str], k: int, k_name: str, xs: list[int]):
    ys = []
    ys += f"""
        (document @uuid {uuids.pop()} @loc "/sa/itihasa/{title_slug}/{k}"
        (copyright CC0. No rights reserved)
        (source "{title} Baroda Critical Edition")
        (title {title} {k_name})
        (sec @x-count
...
def get_kanda_list(uuids: list[str]):
    ys = []
    ys += f"""
        (document @uuid {uuids.pop()} @loc "/sa/itihasa/{title_slug}"
        (copyright CC0. No rights reserved)
        (source "{title} Baroda Critical Edition")
        (title {title})
        (sec @x-count
