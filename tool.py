...
def xml_list_unique_elements(path: str, rel_path: str = '', unique = [], level = 0):
    def traverse(n: sxml.SxmlNode, p: str):
        np = p + '/' + n.id
        if np not in unique:
            unique.append(np)
...
def xml_checker(path: str):
    def write_if_ok(f: fs.File, a: str):
        loc = '/'.join(f.full_path.split('/')[-2:])
        try:
            sxml.from_xml(a)
