...
def xml_list_unique_elements(path: str, rel_path: str = "", unique=dict(), level=0):
    def traverse(n: sxml.SxmlNode, file_path: str):
        ...
        if n.id not in unique:
            unique[n.id] = []
        # unique[n.id].append(file_path)

    for f in fs.list_dirs(path):
        xml_list_unique_elements(
            f.full_path, f.full_path.replace(path, ""), unique, level + 1
        )
...
if __name__ == "__main__":
    if sys.argv[1] == "ocr":
        ...
        from mod.root.backend.importers.tp import gut_download, ramayana, se_download
        ...
        if backend == "se":
            ...
            translator.translate_file(
                sys.argv[3], int(sys.argv[4]) if len(sys.argv) > 4 else 0
            )
        else:
        ...
        for f in fs.list_files(sys.argv[2]):
            fs.write_text(
                f.full_path.replace(".sxml", ".xml"),
                sxml.to_xml(sxml.parse(fs.read_text(f.full_path))),
            )
